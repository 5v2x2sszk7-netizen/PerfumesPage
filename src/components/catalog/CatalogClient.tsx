...
function CatalogToolbar({
  ...
  category: PerfumeCardModel["category"]
  showCount: boolean
  countLabel: string
  ...
  return (
    <Surface className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
        ...
        {showCount ? <p className="hidden text-sm font-medium tracking-wide text-ink-950 sm:block">{countLabel}</p> : null}
      </div>
      ...
      <button
        ...
        className={cn(
          "inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-ink-50/70 px-5 text-sm font-medium tracking-wide text-ink-950 ring-1 ring-inset ring-black/8 transition-luxe duration-luxe-fast ease-luxe hover:-translate-y-0.5 hover:bg-white sm:w-auto",
          focusRing,
            ...
            <div className="flex items-end gap-2 pb-0">
              <button
                ...
                className={cn(
                  ...
                  focusRing,
                  view === "grid" ? "bg-ink-950 text-white ring-white/10" : "bg-ink-50/70 text-ink-950 ring-black/8 hover:bg-white"
                )}
              ...
              <button
                ...
                className={cn(
                  "inline-flex h-10 w-10 items-center justify-center rounded-full ring-1 ring-inset transition duration-500 ease-luxe",
                  focusRing,
                  view === "list" ? "bg-ink-950 text-white ring-white/10" : "bg-ink-50/70 text-ink-950 ring-black/8 hover:bg-white"
                )}
          ...
          <div className="flex items-center gap-3">
            ...
            {hasActiveFilters ? (
              <Button
                type="button"
                variant="ghost"
                className="rounded-full bg-ink-50/70 px-5 py-2.5 text-sm text-ink-700 ring-1 ring-inset ring-black/8 transition duration-luxe-fast ease-luxe hover:bg-white"
  ...
  if (ordered.length) {
    ...
    if (view === "grid" && ordered.length === 2) {
      return (
        <div className="mt-10 mx-auto grid max-w-catalog-grid grid-cols-2 gap-4 sm:gap-6">
          {ordered.map((p, idx) => (
            idx < 12 ? (
              <LazyReveal key={p.id} delayMs={idx * 90} className="h-full w-full">
                <PerfumeCard perfume={p} className="h-full" />
              </LazyReveal>
            ) : (
    ...
    return (
      <div className={cn("mt-8 grid gap-4 sm:gap-6", view === "grid" ? "grid-cols-2 lg:grid-cols-3" : "grid-cols-1")}>
        {ordered.map((p, idx) => (
          idx < 12 ? (
            <LazyReveal key={p.id} delayMs={idx * 55} className="h-full w-full">
              <PerfumeCard perfume={p} className="h-full" />
            </LazyReveal>
          ) : (
            <div key={p.id} className="h-full w-full">
...
export function CatalogClient({ perfumes }: { perfumes: PerfumeCardModel[] }) {
  const { searchParams, replaceQuery } = useCatalogQueryState()
